...
// export default function CustomerPrefences({navigation , route}) {
...
export default function CustomerPreferences({navigation, route}) {
  const {userData} = route.params;
  ...
  const next = () => {
    if (survey === 'onceMonth') {
      if (!select) {
        ErrorShow('error', 'Oops', 'Please select an option');
        return;
      }
      selectSurvey('hairCut');
    } else if (survey === 'hairCut') {
      if (!hairCut && !beardTrim && !shave && !hairStyling) {
        ErrorShow('error', 'Oops', 'Please select at least one service');
        return;
      }
      selectSurvey('contact');
    } else if (survey === 'contact') {
      if (!email && !sms && !notifyApp && !call) {
        ErrorShow(
          'error',
          'Oops',
      ...
      selectSurvey('influence');
    } else if (survey === 'influence') {
      if (!location && !price && !reviewRating && !atmosphere) {
        ErrorShow('error', 'Oops', 'Please select at least one factor');
        return;
      }
      const surveyAnswers = [
        {
        ...
        {
          question:
            'What is your preferred method of communication for appointment reminders and updates?',
          answer: [
            email && 'Email',
            sms && 'SMS text messages',
            notifyApp && 'Push notifications through the app',
            call && 'Calling',
          ].filter(Boolean),
        },
        {
          question:
            'What factors influence your choice of a barber shop? (Select all that apply)',
          answer: [
            location && 'Location',
            price && 'Price',
            reviewRating && 'Reviews and ratings',
            atmosphere && 'Atmosphere/Decor',
          ].filter(Boolean),
        },
      ];
