...
export default function BaberCatalogue({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      if (selectedService) {
        const service = filteredServices?.find(service => service?._id === selectedService?._id)
            ...
            <View>
              <View style={styles.serviceContainer}>
                {filteredServices?.map((item, index) => (
                  <TouchableOpacity
                    style={selectedService?._id === item?._id ? styles.serviceBoxSelected : styles.serviceBox}
                    key={index}
