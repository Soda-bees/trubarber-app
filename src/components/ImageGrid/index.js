...
const ImageGrid = ({ images }) => {
    ...
    const renderSingleImage = () => {
        ...
        if (image) {
          const imageDimensions = singleImageHeights[0] || {
            width: 0,
            height: 0,
          };
          return (
        ...
        if (modalVisible && scrollViewRef.current) {
            setTimeout(() => {
                scrollViewRef.current.scrollTo({
                    x: selectedIndex * sizes.screenWidth,
                    animated: false,
                });
            }, 0);
        }
    ...
    const calculateHeights = async () => {
        try {
            const heightsPromises = images.map((image, index) =>
                checkHeight(image, index),
            );
    ...
    const calculateDimensions = async () => {
        try {
            const dimensionsPromises = images.map((image, index) =>
                checkDimensions(image, index),
            );
    ...
    return (
        <View style={styles.container}>
            {images.length > 3 ? (
                <View style={styles.gridContainer}>{renderImage()}</View>
            ) : (
                <View>{renderSingleImage()}</View>
            )}
            <Modal
